<template>
  <ion-item v-if="bike" :routerLink="'/bike/' + bike.id" :detail="false" class="list-item">
    <div slot="start" :class="bike.locked ? 'dot dot-locked' : 'dot dot-unlocked'"></div>
    <ion-label class="ion-text-wrap">
      <h2>
        {{ bike.fromName }}
        <span class="date">
          <ion-note>{{ bike.date }}</ion-note>
...
.list-item .dot-unlocked {
  background: #36fa46;
}

.list-item .dot-locked {
  background: #ff0000;
}
